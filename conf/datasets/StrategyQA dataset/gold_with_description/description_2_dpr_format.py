...
datatype = 'train'
path = '/Users/uri/Documents/Uri/school/Thesis/Implicit/thesis/DPR/conf/datasets/StrategyQA dataset/gold/strategyQA_'+datatype+'_HN.json'
with open(path) as json_file:
...
for i,sample_in_dpr_data in enumerate(dpr_format_data):
    ...
    for sample_in_labeld_data in labeled_data:
        if sample_in_labeld_data['question'] == question:
            extra_description = sample_in_labeld_data['extra description']
            sample_in_dpr_data['question'] = '{} {}'.format(question,extra_description)
